...
It lists project groups, temporary sessions, archived sessions, linked projects, session file paths, creation time, update time, and message counts. Before deletion, it generates a cleanup plan and requires a second confirmation to reduce accidental deletes.
...
> If a future Codex release changes the local data structure, deletion may be refused with an explanation in the UI. In that case, pause deletion and report the issue first.
...
Thanks to [liuyoumi/codex-history](https://github.com/liuyoumi/codex-history). The deletion strategy and risk checks in this project were inspired by its deletion logic.
...
If you are an advanced user and are comfortable with the command line, the official built-in archive and deletion flows should still be your first choice:

- **Codex CLI**: use `/delete` in the current session to delete it and exit, or run `codex delete` in a terminal to delete a saved session by ID or name. Use `/archive` or `codex archive` when you only want to hide a transcript without deleting it.
- **Windows Desktop / Codex app**: prefer the archive or deletion entry points provided by the desktop app. Removing a project does not delete its history; a thread may remain even after a project directory or worktree is removed.
- **PyCharm / JetBrains Codex plugin**: prefer deleting from the plugin's own session list, such as right-clicking the session name. Treat these IDE sessions as managed by the plugin, and do not assume they are also removed from external CLI or desktop history.
...
**Codex History Manager** provides a more visual local management and supplemental cleanup workflow. It is useful when you need to browse sessions in one place, verify paths, or organize multiple local sessions in bulk.
